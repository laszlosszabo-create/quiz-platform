...
async function main() {
  ...
  for (const t of tables) {
    ...
    if (error) {
      ...
      console.log(`- ${t}:`, data?.map((r:any)=>r.column_name).join(', ') || '(none)')
    }
  }

  // Try insert into quiz_ai_prompts if exists
  ...
  if (existsAi) {
    ...
    const tryOne = async (payload: any) => {
    ...
    // variant A: user_prompt_template
    await tryOne({ quiz_id, lang:'xx', system_prompt:'s', user_prompt_template:'u' })
    // variant B: user_prompt
    await tryOne({ quiz_id, lang:'xy', system_prompt:'s', user_prompt:'u' })
  } else {
