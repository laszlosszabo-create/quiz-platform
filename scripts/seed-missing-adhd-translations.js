const { getSupabaseAdmin } = require('../src/lib/supabase-config');
...
async function seedMissingTranslations() {
  ...
  const translations = [
    ...
    { key: 'landing_guarantee_2', hu: 'Tudományosan validált kérdéssek és módszertan', en: 'Scientifically validated questions and methodology' },
  ...
  for (const t of translations) {
    ...
    if (huError) {
      console.error(`Error inserting HU translation for ${t.key}:`, huError);
    } else {
      console.log(`✓ Inserted HU: ${t.key}`);
    }
    ...
    if (enError) {
      console.error(`Error inserting EN translation for ${t.key}:`, enError);
    } else {
      console.log(`✓ Inserted EN: ${t.key}`);
    }
  }
