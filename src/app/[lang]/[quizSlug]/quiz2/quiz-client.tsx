...
export function QuizClient({
  ...
  const handleEmailSubmitted = async (leadId: string) => {
    ...
    // Check if we need to complete quiz or continue
    const emailGatePosition = featureFlags.email_gate_position || 'end'
    
    if (emailGatePosition === 'end' || isLastQuestion) {
      await completeQuiz(answers)
    } else {
      // Continue to next question if email gate was at start
      setCurrentQuestionIndex(prev => prev + 1)
    }
  }
  ...
  if (isCompleted) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-blue-50 flex items-center justify-center p-2">
        <div className="text-center max-w-sm">
          ...
          <h2 className="text-lg font-bold text-gray-900 mb-2">Kvíz befejezve!</h2>
          <p className="text-sm text-gray-600 mb-3">Eredményeid feldolgozása...</p>
  ...
  if (!currentQuestion) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 flex items-center justify-center p-2">
        <div className="text-center max-w-sm">
          <div className="w-12 h-12 bg-gradient-to-r from-blue-500 to-purple-500 rounded-full flex items-center justify-center mx-auto mb-3 animate-spin">
            <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
            </svg>
          </div>
          <h2 className="text-lg font-bold text-gray-900 mb-1">Kvíz betöltése...</h2>
          <p className="text-sm text-gray-600">Kérjük várjon...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50">
      {/* Compressed Header - reduced all padding/spacing by ~60% */}
      <header className="px-2 py-3 bg-white/80 backdrop-blur-sm border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-4xl mx-auto">
          {/* Quiz branding and navigation - compressed */}
          <div className="flex justify-between items-center mb-3">
            <div className="flex items-center space-x-2">
              ...
              <div>
                <h1 className="text-sm font-semibold text-gray-900">ADHD Gyorsteszt</h1>
          ...
          <div className="relative">
            <div className="w-full bg-gray-200 rounded-full h-1.5 shadow-inner">
              <div 
                className="bg-gradient-to-r from-blue-500 to-purple-600 h-1.5 rounded-full transition-all duration-500 ease-out shadow-sm relative"
      ...
      {/* Question content - compressed spacing */}
      <main className="px-2 py-3">
        <div className="max-w-3xl mx-auto">
          <div className="bg-white rounded-xl shadow-lg shadow-blue-100/50 border border-white/60 backdrop-blur-sm overflow-hidden">
            <div className="p-3 md:p-4">
              <QuestionComponent
                ...
                selectedValue={answers.find(a => a.questionKey === currentQuestion.key)?.value}
          ...
          <div className="flex justify-center items-center space-x-4 mt-3 text-xs text-gray-600">
